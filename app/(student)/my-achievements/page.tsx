...
import { AchievementsTabs, type AchievementsTab } from '@/components/student/AchievementsTabs'
import { ExamProgressChart } from '@/components/student/ExamProgressChart'
import type { Badge, BadgeRarity } from '@/lib/types'
import { ShieldCheck, Award, BookOpen, FileText, CalendarCheck, Trophy, FileCheck } from 'lucide-react'
...
export default async function StudentAchievementsPage({ searchParams }: { searchParams: Promise<{ tab?: string }> }) {
  ...
  const [
    ...
    supabase.from('exam_submissions').select('*, exam:exams(title, total_marks)').eq('student_id', uid).not('submitted_at', 'is', null).order('submitted_at', { ascending: true }),
    ...
    <>
      ...
      <main className="p-6">
        <AchievementsTabs
          ...
          defaultTab={(['badges', 'certificates', 'progress'].includes(tab ?? '') ? tab : 'progress') as AchievementsTab}
        />
      </main>
    </>
  )
