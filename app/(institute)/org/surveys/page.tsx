            ...
            {surveys.map((survey) => (
              <div key={survey.id} className="bg-white rounded-ruwad shadow-card p-6 flex flex-col gap-3 hover:shadow-ruwad transition">
                ...
                <div className="flex items-center gap-2 mt-2 pt-3 border-t border-ruwad-gray/40">
                  <Link href={`/org/surveys/${survey.id}`} className="flex-1 flex items-center justify-center gap-1.5 text-sm font-semibold text-ruwad-blue hover:bg-ruwad-blue/10 px-3 py-2 rounded-ruwad-sm transition">
                    <Pencil size={15} /> تعديل
                  </Link>
                  <Link href={`/org/surveys/${survey.id}/results`} className="flex items-center justify-center gap-1.5 text-sm font-semibold text-ruwad-navy hover:bg-ruwad-gray/30 px-3 py-2 rounded-ruwad-sm transition">
                    <BarChart3 size={15} />
                  </Link>
