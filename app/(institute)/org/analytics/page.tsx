...
// ============================================================================
// لوحة الإحصائيات المتقدمة للمعهد
// كل الاستعلامات تمر عبر RLS بهوية مدير المعهد، فلا يظهر إلا ما هو مُشارَك معه.
// ============================================================================
...
export default async function InstituteAnalyticsPage() {
  ...
  // المدربون المعتمدون + الكورسات المُشارَكة مع المعهد
  const [{ data: members }, { data: shares }] = await Promise.all([
    supabase.from('institute_members').select('user_id, member:profiles!user_id(full_name)').eq('institute_id', institute.id).eq('status', 'approved'),
    supabase.from('resource_institute_shares').select('resource_type, resource_id, trainer_id').eq('institute_id', institute.id),
  ])
  ...
  const sharedCourseIds = (shares ?? []).filter((s) => s.resource_type === 'courses').map((s) => s.resource_id)
  ...
  const [
    ...
    trainerIds.length ? supabase.from('exams').select('id, title, trainer_id, course_id, total_marks, exam_submissions(student_id, score, total_marks, submitted_at)').in('trainer_id', trainerIds) : Promise.resolve({ data: [] as never[] }),
    trainerIds.length ? supabase.from('assignments').select('id, title, trainer_id, assignment_submissions(student_id, score, submitted_at)').in('trainer_id', trainerIds) : Promise.resolve({ data: [] as never[] }),
    trainerIds.length ? supabase.from('challenges').select('id, title, trainer_id, challenge_submissions(student_id, percentage, submitted_at)').in('trainer_id', trainerIds) : Promise.resolve({ data: [] as never[] }),
    trainerIds.length ? supabase.from('attendance_sessions').select('id, trainer_id, course_id, created_at, attendance_records(student_id, status, checked_in_at)').in('trainer_id', trainerIds) : Promise.resolve({ data: [] as never[] }),
  ])
  ...
  // كل تسليمات الامتحانات (نسبة مئوية) + حسب الطالب/المدرب/الكورس
  ...
  // ==================== السلاسل الزمنية (آخر 12 أسبوعاً) ====================
  const WEEKS = 12
  const now = Date.now()
  ...
  const courseRows = courseList.map((c) => {
    ...
    return {
      id: c.id,
      title: c.title,
      trainer: trainerNames.get(c.trainer_id) ?? 'مدرب',
      students,
  ...
  return (
    <>
      <Header title="الإحصائيات المتقدمة" />
      <main className="p-6 flex flex-col gap-6">
        {/* ===== الترويسة + مؤشرات KPI ===== */}
        <div className="relative overflow-hidden rounded-ruwad shadow-ruwad-lg p-8" style={{ background: 'linear-gradient(180deg, #252943 0%, #1a1e33 100%)' }}>
          ...
          <div className="relative flex items-center gap-3 mb-6">
            ...
            <div>
              <h2 className="text-xl font-extrabold text-white">مركز قيادة {institute.name}</h2>
              <p className="text-sm text-white/60">نظرة شاملة لحظية على كل نشاط معهدك — محدَّثة مع كل زيارة</p>
            </div>
          </div>
          <div className="relative grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-3">
            {kpis.map((k) => (
              <div key={k.label} className={`rounded-ruwad-sm p-4 ${k.accent ? 'bg-ruwad-lime' : 'bg-white/10 backdrop-blur'}`}>
                <k.icon size={16} className={k.accent ? 'text-ruwad-navy/60' : 'text-white/50'} />
                <p className={`text-2xl font-extrabold mt-2 ${k.accent ? 'text-ruwad-navy' : 'text-white'}`}>{k.value}</p>
                <p className={`text-[11px] mt-0.5 ${k.accent ? 'text-ruwad-navy/60' : 'text-white/60'}`}>{k.label}</p>
              </div>
            ))}
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 xl:grid-cols-[340px_1fr] gap-6 items-start">
          ...
          <div className="bg-white rounded-ruwad shadow-card p-6 overflow-x-auto">
