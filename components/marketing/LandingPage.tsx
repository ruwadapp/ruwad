...
const FEATURES = [
  ...
  { icon: Flame, title: 'تحديات حيّة', desc: 'مسابقات بأسلوب Kahoot — كل الطلاب يتنافسون بنفس اللحظة.' },
...
export function LandingPage() {
  return (
    <main dir="rtl" className="bg-white overflow-x-clip">
      ...
      {/* ===== HERO ===== */}
      <section className="relative bg-ruwad-gradient animate-hero-glow overflow-hidden pt-32 pb-20 sm:pt-40 sm:pb-28">
        ...
        <div className="relative max-w-6xl mx-auto px-5 sm:px-8 grid lg:grid-cols-2 gap-12 items-center">
          <div className="flex flex-col items-start gap-6 text-center lg:text-right mx-auto lg:mx-0">
            ...
            <h1 className="text-4xl sm:text-5xl lg:text-[3.4rem] font-extrabold text-white leading-[1.15]">
              تدريبك، بروح <span className="text-ruwad-lime">المسابقة</span>
            </h1>
        ...
        <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-5">
          {FEATURES.map((f, idx) => (
            <Reveal key={f.title} delay={idx * 60}>
              <div className="group bg-white border border-ruwad-gray/60 rounded-ruwad p-6 h-full flex flex-col gap-4 hover:shadow-ruwad-lg hover:-translate-y-1.5 hover:border-transparent transition-all duration-300">
                <div className="w-12 h-12 rounded-ruwad-sm bg-ruwad-gradient flex items-center justify-center text-white group-hover:scale-110 transition-transform">
                  <f.icon size={22} />
                </div>
                <div>
          ...
          {STEPS.map((s, idx) => (
            <Reveal key={s.title} delay={idx * 80}>
              <div className="relative">
                <span className="text-5xl font-extrabold text-ruwad-blue/10">{`0${idx + 1}`}</span>
                <h3 className="font-bold text-ruwad-navy text-lg -mt-3">{s.title}</h3>
                <p className="text-sm text-ruwad-navy/60 mt-1.5 leading-relaxed">{s.desc}</p>
              </div>
            </Reveal>
          ))}
        </div>
      ...
      <section className="bg-[#F5F6FA] py-20 sm:py-28 overflow-hidden">
