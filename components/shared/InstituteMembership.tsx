...
interface Membership {
  ...
  institute: { id: string; name: string; institute_code: string } | null
}
...
export function InstituteMembership({ memberRole, userCode }: { memberRole: 'trainer' | 'student'; userCode: string }) {
  ...
  const load = useCallback(async () => {
    ...
    const { data } = await supabase
      ...
      .select('id, status, institute:institutes(id, name, institute_code)')
      .eq('user_id', user!.id)
  ...
  async function requestJoin(e: React.FormEvent) {
    ...
    if (insertError) {
      setError('حدث خطأ أثناء إرسال الطلب (قد يكون لديك طلب سابق لهذا المعهد بالذات)')
      setSubmitting(false)
      return
    }
  ...
  async function leaveOrCancel(membershipId: string, status: Membership['status']) {
    if (!confirm(status === 'approved' ? 'هل تريد مغادرة هذا المعهد؟' : 'إلغاء طلب الانضمام؟')) return
    await supabase.from('institute_members').delete().eq('id', membershipId)
    ...
    <div className="flex flex-col gap-6">
      ...
      <div className="bg-white rounded-ruwad shadow-card p-6">
        <h2 className="text-lg font-bold text-ruwad-navy mb-4 flex items-center gap-2">
          <Building2 size={20} className="text-ruwad-blue" /> عضويتي في المعاهد
        </h2>
        ...
        {loading ? (
          ...
          <div className="flex flex-col gap-3 mb-5">
            {memberships.length === 0 ? (
              <p className="text-ruwad-navy/50 text-sm py-2">لست منضمّاً لأي معهد بعد. يمكنك الانضمام لأكثر من معهد.</p>
            ) : (
                ...
                <div key={membership.id}>
                  {membership.status === 'approved' && (
                    <div className="flex items-center justify-between p-4 rounded-ruwad-sm bg-ruwad-lime/20">
                      ...
                      <button onClick={() => leaveOrCancel(membership.id, membership.status)} className="flex items-center gap-1.5 text-sm font-semibold text-red-500 shrink-0">
                  ...
                  {membership.status === 'pending' && (
                    <div className="flex items-center justify-between p-4 rounded-ruwad-sm bg-ruwad-gray/30">
                      ...
                      <button onClick={() => leaveOrCancel(membership.id, membership.status)} className="text-sm font-semibold text-red-500 shrink-0">إلغاء الطلب</button>
                    </div>
                  )}
                  {membership.status === 'rejected' && (
                    <div className="flex items-center justify-between p-4 rounded-ruwad-sm bg-red-50">
                      ...
                      <button onClick={() => leaveOrCancel(membership.id, membership.status)} className="text-sm font-semibold text-ruwad-navy/60 shrink-0">إغلاق</button>
                    </div>
                  )}
                </div>
              ))
        ...
        <form onSubmit={requestJoin} className="flex flex-col gap-3 max-w-sm pt-2 border-t border-ruwad-gray/40">
          <p className="text-sm font-medium text-ruwad-navy mt-3">الانضمام لمعهد آخر</p>
